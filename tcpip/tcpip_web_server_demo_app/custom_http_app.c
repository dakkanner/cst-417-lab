...
static HTTP_IO_RESULT HTTPPostConfig(HTTP_CONN_HANDLE connHandle)
{
    ...
    while(HTTPCurConnectionByteCountGet(connHandle))
    {
        ...
        else if(!strcmp((char*)httpDataBuff, (const char*)"mac"))
        {
            ...
            for(i = 0; i < 6u; i++)
            {
                ...
                // On boards that do not have a writable File System
                // (like the Ethernet Starter Kit)
                // this new value cannot be stored and is ignored
            }
        }
    ...
    if(bConfigFailure == false)
    {
        ...
        memcpy((void*)(httpDataBuff+20), TCPIP_STACK_NetBIOSName(currNet), 16);
...
static HTTP_IO_RESULT HTTPPostSNMPCommunity(HTTP_CONN_HANDLE connHandle)
{
	...
	switch(HTTPCurConnectionPostSmGet(connHandle))
	{
		...
		case SM_CFG_SNMP_READ_VALUE:
			// Read a value
			if(HTTPReadPostValue(connHandle, httpDataBuff + 6, HTTP_MAX_DATA_LEN) == HTTP_READ_INCOMPLETE)
                return HTTP_IO_NEED_DATA;
			...
			if(memcmp((void*)httpDataBuff, (const void*)"rcm", 3) == 0)
                        {
                            dest = SNMPRetrieveReadCommunity(vCommunityIndex);
                        }
			else if(memcmp((void*)httpDataBuff, (const void*)"wcm", 3) == 0)
                        {
                            dest = SNMPRetrieveWriteCommunity(vCommunityIndex);
                        }
			else
                        {
                            break;
...
void HTTPPrint_read_comm(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
	...
	// Send proper string
    TCPPutString(sktHTTP, SNMPRetrieveReadCommunity(num));
	#endif
}
...
void HTTPPrint_write_comm(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
	...
	// Send proper string
	TCPPutString(sktHTTP, SNMPRetrieveWriteCommunity(num));
	#endif
}
