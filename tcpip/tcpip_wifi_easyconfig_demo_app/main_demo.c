...
#if defined(TCPIP_IF_MRF24W)
extern void WF_ProcessEvent(uint16_t event, uint16_t eventInfo, uint8_t *p_extraInfo);
#endif
#endif  // defined(TCPIP_STACK_USE_EVENT_NOTIFICATION)
...
static void ProcessNotification(const void* hIf)
{
    ...
    if (mrf_mgmtEvents)
    {
        WF_ProcessEvent(mrf_mgmtEvents, mrf_mgmtEventInfo, NULL);
    }
#endif
}
#endif  // defined(TCPIP_STACK_USE_EVENT_NOTIFICATION)
