...
bool SYS_Initialize(void)
{
    ...
    if(!SYS_RTCC_Initialize(0x10073000, 0x13042302))
    {
        return false;
    }
    // RAM disk need format first
    #if defined(MEDIA_STORAGE_MDD_INTERNAL_RAM)
    if (SYS_FS_createMBR(1, MDD_INTERNAL_RAM_TOTAL_SECTORS, "mdd") != RETURN_SUCCESS)
    {
        return false;
    }
    if (SYS_FS_format(1, 0xffff0001, "RAMDISK", "mdd")!=RETURN_SUCCESS)
    {
        return false;
    }
    #endif
    ...
    if(!SYS_FS_initialize(&ReplaceMe))
    {
        return false;
    }
...
/*********************************************************************
 * Function:        bool SYS_RTCC_Initialize(uint32_t time, uint32_t date)
 *
 * PreCondition:    SYS_TICK_Initialize() must be called prior to this, because
 *                  time-out timer is using SYS_TICK
 *
 * Input:           time    - the time value to be set
 *                  date - the date value to be set
 *
 ...
 * Side Effects:    None
 *
 * Overview:        Initializes the system Real Timer Clock.
 *
 * Note:            This would depends on your HW settings, like 32K crystal
 ********************************************************************/
bool SYS_RTCC_Initialize(uint32_t time, uint32_t date)
{
    uint32_t cnt = 0;
#if defined (__C32__)
    RtccInit();
...
/*********************************************************************
 * Function:        bool SYS_RTCC_DateGet(void)
 *
 * PreCondition:    SYS_RTCC_Initialize() already called
 *
 * Input:           None
 *
 * Output:          The current date of the RTCC device.
 *                  Can be safely cast into rtccDate
 *
 * Side Effects:    None
 *
 * Overview:        Initializes the system Real Timer Clock.
 ...
 * Note:
 ********************************************************************/
uint32_t SYS_RTCC_DateGet(void)
{
#if defined (__C32__)
    return RtccGetDate();
#endif
...
/*********************************************************************
 * Function:        bool SYS_RTCC_TimeGet(void)
 *
 * PreCondition:    SYS_RTCC_Initialize() already called
 *
 * Input:           None
 *
 * Output:          The current value of the time
 *                  Can be safely casted to an rtccTime structure
 *
 * Side Effects:    None
 *
 * Overview:        Initializes the system Real Timer Clock.
 ...
 * Note:
 ********************************************************************/
uint32_t SYS_RTCC_TimeGet(void)
{
#if defined (__C32__)
    return RtccGetTime();
#endif
