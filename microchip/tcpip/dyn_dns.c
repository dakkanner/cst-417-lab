...
bool DDNSInit(const TCPIP_STACK_MODULE_CTRL* const stackData,
              const DDNS_MODULE_GONFIG* ddnsData)
{
...
void DDNSTask(void)
{
	...
	switch(smDDNS)
	{
        ...
        case SM_DNS_START_RESOLVE:
            ...
            {
                DNSResolve((const char*)DDNSClient.CheckIPServer.szROM, DNS_TYPE_A);
            }
            else
            {
                DNSResolve((const char*)DDNSClient.CheckIPServer.szRAM, DNS_TYPE_A);
            }
        ...
        case SM_DNS_WAIT_RESOLVE:
			...
			if(DDNSClient.ROMPointers.CheckIPServer)
            {
                dnsRes = DNSIsResolved((const char*)DDNSClient.CheckIPServer.szROM, &ddnsServerIP);
            }
            else
            {
                dnsRes = DNSIsResolved((const char*)DDNSClient.CheckIPServer.szRAM, &ddnsServerIP);
            }
            ...
            MySocket = TCPOpenClient(IP_ADDRESS_TYPE_IPV4, DDNSClient.CheckIPPort, (IP_MULTI_ADDRESS*)&ddnsServerIP);
		...
		case SM_IP_UPDATE_HOME:
            ...
            {
                DNSResolve((const char*)DDNSClient.UpdateServer.szROM, DNS_TYPE_A);
            }
            else
            {
                DNSResolve((const char*)DDNSClient.UpdateServer.szRAM, DNS_TYPE_A);
            }
        ...
        case SM_IP_UPDATE_WAIT_DNS:
			...
			if(DDNSClient.ROMPointers.UpdateServer)
            {
                dnsRes = DNSIsResolved((const char*)DDNSClient.UpdateServer.szROM, &ddnsUpdateIP);
            }
            else
            {
                dnsRes = DNSIsResolved((const char*)DDNSClient.UpdateServer.szRAM, &ddnsUpdateIP);
            }
            ...
            MySocket = TCPOpenClient(IP_ADDRESS_TYPE_IPV4, DDNSClient.UpdatePort, (IP_MULTI_ADDRESS*)&ddnsUpdateIP);
